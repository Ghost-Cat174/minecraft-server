...
echo You WILL be installing a minecraft 1.19 server!
...
sudo rm -rf $path && sudo mkdir $path && cd $path && sudo wget https://api.papermc.io/v2/projects/paper/versions/1.19.2/builds/133/downloads/paper-1.19.2-133.jar -O paper.jar
cd $path && wget https://github.com/KittyCat951/minecraft-server/releases/download/update/run.sh && chmod +x run.sh
cd $path && wget https://github.com/KittyCat951/minecraft-server/releases/download/server.propeties/server-1.19.properties -O server.properties
...
echo TO RUN THE SERVER AGAIN TYPE cd $path && sudo ./run TO SAFLY RUN THE SERVER!
