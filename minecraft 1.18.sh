...
echo You WILL be installing a minecraft 1.18 server!
...
sudo rm -rf $path && sudo mkdir $path && cd $path && sudo wget https://api.papermc.io/v2/projects/paper/versions/1.18.2/builds/387/downloads/paper-1.18.2-387.jar -O paper.jar
cd $path && wget https://github.com/KittyCat951/minecraft-server/releases/download/update/run.sh && chmod +x run.sh
cd $path && wget https://github.com/KittyCat951/minecraft-server/releases/download/server.propeties/server-1.18.properties -O server.properties
